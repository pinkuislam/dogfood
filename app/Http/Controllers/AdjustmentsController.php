		...
		if ($user_auth->can('adjustment_view_all') || $user_auth->can('adjustment_view_own')){
            ...
            if ($request->ajax()) {
                ...
                ->addColumn('action', function($row) use ($user_auth) {
                    $btn = '';
                    if ($user_auth->can('adjustment_view_all')){
                        $btn .= '<a href="'.url('adjustment/adjustments/' .$row->id).'" id="' .$row->id. '"  class="cursor-pointer ul-link-action text-info"
                        data-bs-toggle="tooltip" data-placement="top" title="View"><i class="far fa-eye"></i></a>';
                         $btn .= '&nbsp;&nbsp;';
                    }
                    // if ($user_auth->can('adjustment_edit')){
                    //     $btn .= '<a href="'.url('adjustment/adjustments/' .$row->id. '/edit').'" id="' .$row->id. '"  class="edit cursor-pointer ul-link-action text-success"
                    //     data-bs-toggle="tooltip" data-placement="top" title="Edit"><i class="far fa-edit"></i></a>';
                    //      $btn .= '&nbsp;&nbsp;';
                    // }
                    // if ($user_auth->can('adjustment_delete')){
                    //     $btn .= '<a id="' .$row->id. '" class="delete cursor-pointer ul-link-action text-danger mr-1"
                    //     data-bs-toggle="tooltip" data-placement="top" title="Remove"><i class="far fa-times-circle"></i></a>';
                    //      $btn .= '&nbsp;&nbsp;';
                    // }
                    return $btn;
                })
    ...
    public function store(Request $request)
    {
		...
		if ($user_auth->can('adjustment_add')){
            ...
            \DB::transaction(function () use ($request) {
                ...
                $order->Ref = 'AD-' . date("Ymd") . '-'. date("his");
                        ...
                        if ($value['product_variant_id']) {
                            ...
                            if ($ProductWarehouse) {
                                $inventoryLog = array(
                                    ...
                                    'new_quantity' => ($ProductWarehouse->qty + $value['quantity']),
                                    'type' => 'AD',
                                    'ref_id' => $order->id,
    ...
    public function show($id){
		...
		if ($user_auth->can('adjustment_view_all') || $user_auth->can('adjustment_view_own')){
            ...
            if ($Adjustment_data->warehouse_id) {
                $warehouse = Warehouse::where('id', $Adjustment_data->warehouse_id)
                ->where('deleted_at', '=', null)
                ->first();
                if ($warehouse) {
                    $adjustment['warehouse_id'] = $Adjustment_data->warehouse_id;
                    $adjustment['warehouse'] = $warehouse->name;
                } else {
            ...
            return view('adjustments.details_adjustment',[
                'details' => $details,
                    ...
                    if ($product_detail['type'] == "add") {
                        ...
                        if ($product_detail['product_variant_id']) {
                            ...
                            if ($ProductWarehouse) {
                                ...
                                $ProductWarehouse->qty += $product_detail['quantity'];
                                $ProductWarehouse->save();
                            }
