...
class PurchasesReturnController extends Controller
{
    ...
    public function index(Request $request)
    {
		...
		if ($user_auth->can('purchase_returns_view_all') || $user_auth->can('purchase_returns_view_own')){
            ...
            if ($request->ajax()) {
                ...
                $data = PurchaseReturn::where('deleted_at', '=', null)
                    ...
                    })->with('purchase','facture', 'supplier', 'warehouse')
                    ->orderBy('id', 'desc');
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 34px, 0px);">
                                    ...
                                    //check if user has permission "purchase_returns_edit"
                                     ...
                                     if ($user_auth->can('purchase_returns_edit')){
                                        $btn .=  '<a class="dropdown-item" href="/purchase-return/edit_returns_purchase/' .$row->id. '/'.$row->purchase_id.'" ><i class="nav-icon far fa-edit font-weight-bold mr-2"></i> '.trans('translate.Edit_Return').'</a>';
                                    }
                                    ...
                                    if ($user_auth->can('purchase_returns_delete')){
                                        $btn .= '<a class="dropdown-item delete cursor-pointer" id="' .$row->id. '" > <i class="nav-icon far fa-times-circle font-weight-bold mr-2"></i> ' .trans('translate.Delete_Return').'</a>';
                                    }
                                    $btn .= '</div>
                            </div>';
     ...
     public function create_purchase_return(Request $request , $id)
     {
         ...
         if ($user_auth->can('purchase_returns_add')){
             ...
             $Return_detail['supplier_id']  = $PurchaseReturn->supplier_id;
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($user_auth->can('purchase_returns_view_all') || $user_auth->can('purchase_returns_view_own')){
            ...
            $return_details['supplier_name']  = $Purchase_Return['supplier']->name;
            $return_details['supplier_email'] = $Purchase_Return['supplier']->email;
            $return_details['supplier_phone'] = $Purchase_Return['supplier']->phone;
            $return_details['supplier_adr']   = $Purchase_Return['supplier']->address;
    ...
    public function edit_purchase_return(Request $request, $id, $purchase_id)
    {
        ...
        if ($user_auth->can('purchase_returns_edit')){
            ...
            $Return_detail['supplier_id']            = $Purchase_Return->supplier_id;
                        ...
                        if ($product_detail['product_variant_id']) {
                            ...
                            if ($unit_prod && $ProductWarehouse) {
                                if ($unit_prod->operator == '/') {
                                    $ProductWarehouse->qte -= $product_detail['quantity'] / $unit_prod->operator_value;
                                } else {
                                    $ProductWarehouse->qte -= $product_detail['quantity'] * $unit_prod->operator_value;
                                }
                                $ProductWarehouse->save();
    ...
    public function destroy($id)
    {
     ...
     public function Send_Email(Request $request)
     {
        ...
        $id = $request->id;
         $purchase_return_data = PurchaseReturn::with('supplier')
         ->where('deleted_at', '=', null)
         ->findOrFail($id);
         ...
         $purchase_return['to'] = $purchase_return_data['supplier']->email;
         $purchase_return['supplier_name'] = $purchase_return_data['supplier']->name;
    ...
    public function getNumberOrder()
    {
     ...
     public function Return_pdf(Request $request, $id)
     {
         ...
         $return_details['supplier_name']    = $PurchaseReturn['supplier']->name;
         $return_details['supplier_phone']   = $PurchaseReturn['supplier']->phone;
         $return_details['supplier_adr']     = $PurchaseReturn['supplier']->address;
         $return_details['supplier_email']   = $PurchaseReturn['supplier']->email;
