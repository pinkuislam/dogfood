...
class SalesController extends Controller
{
    ...
    public function index(Request $request)
    {
		...
		if ($user_auth->can('sales_view_all') || $user_auth->can('sales_view_own')){
            ...
            return view('sales.list_sales',compact('clients','warehouses','currency','symbol_placement'));
            ...
            foreach ($sales as $sale) {
                ...
                $item['action'] = '<div class="dropdown">
                        ...
                        if ($user_auth->can('sales_edit') &&  $sale_has_return == 'no'){
                            $item['action'] .= '<a class="dropdown-item" href="/sale/sales/' .$sale->id. '/edit" ><i class="nav-icon far fa-edit font-weight-bold mr-2"></i> ' .trans('translate.EditSale').'</a>';
                        }
                        ...
                        if ($user_auth->can('sales_delete') &&  $sale_has_return == 'no'){
                            $item['action'] .= '<a class="dropdown-item delete cursor-pointer" id="' .$sale->id. '" > <i class="nav-icon far fa-times-circle font-weight-bold mr-2"></i> ' .trans('translate.DeleteSale').'</a>';
                        }
    ...
    public function store(Request $request)
    {
		...
		if ($user_auth->can('sales_add')){
            ...
            \DB::transaction(function () use ($request) {
                ...
                $order->Ref = 'INV-' . date("Ymd") . '-'. date("his");
            ...
            if ($user_auth->can('sales_edit')){
                ...
                foreach ($get_ProductWarehouse_data as $ProductWarehouse) {
                    ...
                    if ($ProductWarehouse->product_variant_id) {
                        ...
                        $item['code'] = $ProductWarehouse['productVariant']->code;
                        ...
                        $item['barcode'] = $ProductWarehouse['productVariant']->code;
                    ...
                    if($ProductWarehouse['product']['unitSale']){
                        ...
                        if($ProductWarehouse['product']['unitSale']->operator == '/') {
                            $item['qte_sale'] = $ProductWarehouse->qte * $ProductWarehouse['product']['unitSale']->operator_value;
                        ...
                        $item['qte_sale'] = $ProductWarehouse->qte;
                        $price = $product_price;
                    }
                    ...
                    if($ProductWarehouse['product']['unitPurchase']) {
                        ...
                        if($ProductWarehouse['product']['unitPurchase']->operator == '/') {
                            $item['qte_purchase'] = round($ProductWarehouse->qte * $ProductWarehouse['product']['unitPurchase']->operator_value, 5);
                        ...
                        $item['qte_purchase'] = $ProductWarehouse->qte;
                    }


                    $item['qte'] = $ProductWarehouse->qte;
                ...
                \DB::transaction(function () use ($request, $id) {
                    ...
                    foreach ($old_sale_details as $key => $value) {
                        ...
                        $old_unit = Unit::where('id', $value['sale_unit_id'])->first();
                            ...
                            if ($value['product_variant_id']) {
                                ...
                                if ($ProductWarehouse && $old_unit) {
                                    if ($old_unit->operator == '/') {
                                        $ProductWarehouse->qte += $value['quantity'] / $old_unit->operator_value;
                                    } else {
                                        $ProductWarehouse->qte += $value['quantity'] * $old_unit->operator_value;
                                    }
                                    $ProductWarehouse->save();
                    ...
                    foreach ($new_sale_details as $prd => $prod_detail) {
                            ...
                            if ($prod_detail['product_variant_id']) {
                                ...
                                if ($ProductWarehouse && $unit_prod) {
                                    if ($unit_prod->operator == '/') {
                                        $ProductWarehouse->qte -= $prod_detail['quantity'] / $unit_prod->operator_value;
                                    } else {
                                        $ProductWarehouse->qte -= $prod_detail['quantity'] * $unit_prod->operator_value;
                                    }
                                    $ProductWarehouse->save();
