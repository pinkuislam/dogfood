...
class ReportController extends Controller
{
    ...
    public function get_report_clients_datatable(Request $request)
    {
      ...
      public function report_suppliers(Request $request)
      {
          $user_auth = auth()->user();
          if ($user_auth->can('report_fournisseurs')){
              ...
              return view('reports.report_suppliers', compact('warehouses'));
    ...
    //------------ get_report_suppliers_datatable-----------\\

    public function get_report_suppliers_datatable(Request $request)
    {
        $user_auth = auth()->user();
        if (!$user_auth->can('report_fournisseurs')){
            return abort('403', __('You are not authorized'));
            ...
            $start = $request->input('start');
            $order = 'suppliers.'.$columns_order[$request->input('order.0.column')];
            $dir = $request->input('order.0.dir');

            $suppliers_data = Supplier::where('deleted_at', '=', null)
            ->where(function ($query) use ($user_auth) {
            ...
            $totalRows = $suppliers_data->count();
            $totalFiltered = $totalRows;
    ...
    public function get_payment_purchase_report_datatable(Request $request)
    {
        ...
        if (!$user_auth->can('payment_sale_reports')){
            ...
            ->whereDate('date', '>=', $start_date)
            ->whereDate('date', '<=', $end_date)
            ->with('purchase.supplier','purchase.warehouse','payment_method','account')
            ...
            // Multiple Filter
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('supplier_id'), function ($query) use ($request) {
                    return $query->whereHas('purchase.supplier', function ($q) use ($request) {
            ...
            ->where(function ($query) use ($request) {
             ...
             foreach ($payments as $payment) {
                 ...
                 $item['supplier_name']  = $payment->purchase->supplier->name;
            ...
            if(empty($request->warehouse_id)){
             ...
             if ($ProductWarehouse_data->isNotEmpty()) {
                 ...
                 foreach ($ProductWarehouse_data as $ProductWarehouse) {
                     if ($ProductWarehouse->qte <= $ProductWarehouse['product']->stock_alert) {
                         if ($ProductWarehouse->product_variant_id !== null) {
                             $item['product_code'] = $ProductWarehouse['productVariant']->code;
                             $item['product_name'] = '['.$ProductWarehouse['productVariant']->name . '] ' . $ProductWarehouse['product']->name;

                         } else {
                             $item['product_code'] = $ProductWarehouse['product']->code;
                             $item['product_name'] = $ProductWarehouse['product']->name;
                         }
                         $item['current_stock'] = $ProductWarehouse->qte;
                         $item['product_id'] = $ProductWarehouse['product']->id;
                         $item['warehouse_name'] = $ProductWarehouse['warehouse']->name;
     ...
     public function payment_purchase_return_report(Request $request)
     {
		...
		if ($user_auth->can('payment_return_purchase_reports')){
            ...
            if ($request->ajax()) {
                ...
                ->whereDate('date', '>=', $start_date)
                ->whereDate('date', '<=', $end_date)
                ->with('PurchaseReturn', 'PurchaseReturn.supplier','payment_method','account')
                ->orderBy('id', 'desc')
    
                // Multiple Filter
                ->where(function ($query) use ($request) {
                    return $query->when($request->filled('supplier_id'), function ($query) use ($request) {
                        return $query->whereHas('PurchaseReturn.supplier', function ($q) use ($request) {
                            $q->where('id', '=', $request->supplier_id);
                        });
                    });
                });
                ...
                ->addColumn('supplier_name', function($row){
                    return $row->PurchaseReturn->supplier->name;
                })
                ->addColumn('montant', function($row){
                    return $this->render_price_with_symbol_placement(number_format($row->montant, 2, '.', ','));
                })
   ...
   public function report_profit_filter(Request $request  , $start_date , $end_date , $warehouse)
   {
    ...
    public function purchases_history_page(Request $request)
    {
		...
		if ($user_auth->can('purchase_reports')){
            ...
            return view('reports.purchases_history',compact('suppliers','products','warehouses'));
    ...
    public function get_purchases_history_datatable(Request $request)
    {
        ...
        if (!$user_auth->can('purchase_reports')){
            ...
            $purchase_details_data = PurchaseDetail::with('product','purchase','purchase.supplier','purchase.warehouse')
            ...
            foreach ($purchase_details as $detail) {
                ...
                if($detail->purchase_unit_id !== null){
                  ...
                  $item['supplier_name']  = $detail['purchase']['supplier']->name;
