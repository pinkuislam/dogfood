...
class PurchasesController extends Controller
{
    ...
    public function index(Request $request)
    {
		...
		if ($user_auth->can('purchases_view_all') || $user_auth->can('purchases_view_own')){
            ...
            $suppliers = supplier::where('deleted_at', '=', null)->get(['id', 'name']);
            ...
            return view('purchases.list_purchases',compact('suppliers','warehouses','currency','symbol_placement'));
        ...
        if (!$user_auth->can('purchases_view_all') && !$user_auth->can('purchases_view_own')){
                ...
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where('Ref', 'LIKE', "%{$request->input('search.value')}%")
                        ...
                        ->orWhere(function ($query) use ($request) {
                            return $query->whereHas('supplier', function ($q) use ($request) {
            ...
            foreach ($purchases as $purchase) {
                ...
                if (PurchaseReturn::where('purchase_id', $purchase->id)->where('deleted_at', '=', null)->exists()) {
                    ...
                    $item['Ref']         = $purchase->Ref .' '.'<i class="text-15 text-danger fas fa-exchange-alt"></i>';
                }else{
                ...
                $item['action'] = '<div class="dropdown">
                                <button class="btn btn-outline-info btn-rounded dropdown-toggle" id="dropdownMenuButton" type="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'
                                ...
                                if ($user_auth->can('purchases_details')){
                                    $item['action'] .=    '<a class="dropdown-item" href="'.url('purchase/purchases/' .$purchase->id).'"> <i class="nav-icon far fa-eye font-weight-bold mr-5"></i> ' .trans('translate.PurchaseDetail').'</a>';
                                }

                                if ($user_auth->can('purchases_edit') &&  $purchase_has_return == 'no'){
                                    $item['action'] .=    '<a class="dropdown-item" href="'.url('purchase/purchases/' .$purchase->id. '/edit').'" ><i class="nav-icon far fa-edit font-weight-bold mr-5"></i> ' .trans('translate.EditPurchase').'</a>';
                                }

                                //check if user has permission "payment_purchases_view"
                                if ($user_auth->can('payment_purchases_view')){
                                    $item['action'] .=    '<a class="dropdown-item Show_Payments cursor-pointer"  id="' .$purchase->id. '" > <i class="nav-icon fas fa-credit-card font-weight-bold mr-5"></i> ' .trans('translate.ShowPayment').'</a>';
                                }
                                //check if user has permission "payment_purchases_add"
                                if ($user_auth->can('payment_purchases_add')){
                                    $item['action'] .=    '<a class="dropdown-item New_Payment cursor-pointer" payment_status="' .$purchase->payment_status. '"  id="' .$purchase->id. '" > <i class="nav-icon fas fa-dollar-sign font-weight-bold mr-5"></i> ' .trans('translate.AddPayment').'</a>';
                                }
                                $item['action'] .=  '<a class="dropdown-item download_pdf cursor-pointer" Ref="' .$purchase->Ref. '" id="' .$purchase->id. '" ><i class="nav-icon far fa-file-pdf font-weight-bold mr-5"></i> ' .trans('translate.DownloadPdf').'</a>';
                                
                                //$item['action'] .=  '<a class="dropdown-item  send_email cursor-pointer" id="' .$purchase->id. '" ><i class="nav-icon far fa-envelope font-weight-bold mr-5"></i> ' .trans('translate.EmailPurchase').'</a>';

                                //check if user has permission "purchases_delete"
                                if ($user_auth->can('purchases_delete') &&  $purchase_has_return == 'no'){
                                    $item['action'] .= '<a class="dropdown-item delete cursor-pointer" id="' .$purchase->id. '" > <i class="nav-icon far fa-times-circle font-weight-bold mr-5"></i> ' .trans('translate.DeletePurchase').'</a>';
                                }
                                $item['action'] .= '</div>
                            </div>';
                    ...
                    if ($value['product_variant_id']) {
                        ...
                        if ($unit && $ProductWarehouse) {
                            $inventoryLog = array(
                                ...
                                'old_quantity' => $ProductWarehouse->qty,
                                'change_quantity' => $value['quantity'],
                                'new_quantity' => ($ProductWarehouse->qty + ($unit->operator == '/' ? ($value['quantity'] / $unit->operator_value) : ($value['quantity'] * $unit->operator_value))),
                                'type' => 'PO',
                                'ref_id' => $order->id,
                                'created_by' => Auth::user()->id,
                                'created_at' => Carbon::now()
                            );
                            InventoryLog::insert($inventoryLog);
    ...
    public function show($id)
    {
		...
		if ($user_auth->can('purchases_details')){
            ...
            $purchase_data['supplier_name']          = $purchase['supplier']->name;
            $purchase_data['supplier_email']         = $purchase['supplier']->email;
            $purchase_data['supplier_phone']         = $purchase['supplier']->phone;
            $purchase_data['supplier_adr']           = $purchase['supplier']->address;
            ...
            foreach ($purchase['details'] as $detail) {
                ...
                if ($detail->product_variant_id) {
                    ...
                    $data['sku'] = $productsVariants->sku;
                    $data['name'] = '['.$productsVariants->name . '] ' . $detail['product']['name'];
            
                } else {
                    $data['sku'] = $detail['product']['sku'];
        ...
        if (PurchaseReturn::where('purchase_id', $id)->where('deleted_at', '=', null)->exists()) {
            ...
            if ($user_auth->can('purchases_edit')){
                ...
                if ($Purchase_data->supplier_id) {
                    if (Supplier::where('id', $Purchase_data->supplier_id)->where('deleted_at', '=', null)->first()) {
                ...
                $products_array = [];
                $get_ProductWarehouse_data = ProductWarehouse::with('warehouse', 'product', 'productVariant')
                    ->where('warehouse_id', $Purchase_data->warehouse_id)
                    ->where('deleted_at', '=', null)
                    ->where('manage_stock', true)
                    ->get();

                foreach ($get_ProductWarehouse_data as $ProductWarehouse) {
                ...
                \DB::transaction(function () use ($request, $id) {
                        ...
                        if ($value['product_variant_id']) {
                            ...
                            if ($unit && $ProductWarehouse) {
                                $inventoryLog = array(
                                    ...
                                    'product_variant_id' => $value['product_variant_id'],
                                    'old_quantity' => $ProductWarehouse->qty,
                                    'change_quantity' => -1*$value['quantity'],
                                    'new_quantity' => ($ProductWarehouse->qty - ($unit->operator == '/' ? ($value['quantity'] / $unit->operator_value) : ($value['quantity'] * $unit->operator_value))),
                                    'type' => 'PO_AD',
                                    'ref_id' => $current_Purchase->id,
                                    'created_by' => Auth::user()->id,
                                    'created_at' => Carbon::now()
                                );
                                InventoryLog::insert($inventoryLog);
                    ...
                    foreach ($new_purchase_details as $key => $prod_detail) {
                        ...
                        if ($prod_detail['product_variant_id']) {
                            ...
                            if ($unit_prod && $ProductWarehouse) {
                                if ($unit_prod->operator == '/') {
                                    $ProductWarehouse->qty += $prod_detail['quantity'] / $unit_prod->operator_value;
                                } else {
                                    $ProductWarehouse->qty += $prod_detail['quantity'] * $unit_prod->operator_value;
                                }
            ...
            if ($user_auth->can('purchases_delete')){
                ...
                \DB::transaction(function () use ($id) {
                    ...
                    foreach ($payments as $payment) {
                        ...
                        if ($account) {
                            $account->update([
                                'initial_balance' => $account->initial_balance + $payment->amount,
                            ]);
                        }
    ...
    public function Purchase_pdf(Request $request, $id)
    {
        ...
        $purchase['supplier_name']  = $Purchase_data['supplier']->name;
        $purchase['supplier_phone'] = $Purchase_data['supplier']->phone;
        $purchase['supplier_adr']   = $Purchase_data['supplier']->address;
        $purchase['supplier_email'] = $Purchase_data['supplier']->email;
    ...
    {
        //purchase
        $purchase = Purchase::with('supplier')->where('deleted_at', '=', null)->findOrFail($request->id);
        ...
        //receiver email 
        $receiver_email = $purchase['supplier']->email;
    ...
    public function get_Products_by_purchase(Request $request , $id)
    {
        ...
        $Return_detail['supplier_id'] = $Purchase_data->supplier_id;
        ...
        foreach ($Purchase_data['details'] as $detail) {
            ...
            if ($detail->product_variant_id) {
                ...
                $item_product ? $data['del'] = 0 : $data['del'] = 1;
                $data['sku'] = $productsVariants->name . '-' . $detail['product']['sku'];
                $data['product_variant_id'] = $detail->product_variant_id;                
