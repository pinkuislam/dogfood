...
class SuppliersController extends Controller
{
    ...
    public function index(Request $request)
    {
		...
		if ($user_auth->can('suppliers_view_all') || $user_auth->can('suppliers_view_own')){
            ...
            return view('suppliers.suppliers_list', ['currency' => $this->currency, 'symbol_placement' => $this->symbol_placement]);
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 34px, 0px);">';
                                ...
                                //check if user has permission "supplier_details"
                                 if ($user_auth->can('supplier_details')){
                                    $item['action'] .=  ' <a class="dropdown-item" href="'.url('products/suppliers/' .$supplier->id).'"> <i class="nav-icon far fa-eye font-weight-bold mr-2"></i> ' .trans('translate.Supplier_details').'</a>';
                                }

                                //check if user has permission "suppliers_edit"
                                if ($user_auth->can('suppliers_edit')){
                                    $item['action'] .=  '<a class="dropdown-item edit cursor-pointer" id="' .$supplier->id. '"><i class="nav-icon far fa-edit font-weight-bold mr-2"></i> ' .trans('translate.Edit_Supplier').'</a>';
                                }

                                //check if user has permission "pay_purchase_due"
                                /*if ($user_auth->can('pay_purchase_due')){
                                    $item['action'] .=  '<a class="dropdown-item payment_purchase cursor-pointer"  id="' .$supplier->id. '" > <i class="nav-icon fas fa-dollar-sign font-weight-bold mr-2"></i> ' .trans('translate.pay_all_purchase_due_at_a_time').'</a>';
                                }

                                //check if user has permission "pay_purchase_return_due"
                                if ($user_auth->can('pay_purchase_return_due')){
                                    $item['action'] .=  '<a class="dropdown-item payment_return cursor-pointer"  id="' .$supplier->id. '" > <i class="nav-icon fas fa-dollar-sign font-weight-bold mr-2"></i> ' .trans('translate.pay_all_purchase_return_due_at_a_time').'</a>';
                                }*/

                                //check if user has permission "suppliers_delete"
                                if ($user_auth->can('suppliers_delete')){
                                    $item['action'] .=  '<a class="dropdown-item delete cursor-pointer" id="' .$supplier->id. '" > <i class="nav-icon far fa-times-circle font-weight-bold mr-2"></i> ' .trans('translate.Delete_Supplier').'</a>';
                                }
                                $item['action'] .=  '</div>
                        </div>';
                    $data[] = $item;
    ...
    public function getNumberOrder()
    {
        ...
        $last = DB::table('suppliers')->latest('id')->first();
    ...
    public function get_supplier_debt_total($id){
    ...
    public function suppliers_pay_due(Request $request)
    {
        $user_auth = auth()->user();
		if ($user_auth->can('pay_purchase_due')){
            ...
            if($request['amount'] > 0){
                $supplier_purchases_due = Purchase::where('deleted_at', '=', null)
                ->where([
                    ['payment_statut', '!=', 'paid'],
     ...
     //------------- get_supplier_debt_return_total -------------\\

    public function get_supplier_debt_return_total($id){
     ...
     //------------- suppliers_pay_return_due -------------\\

     public function suppliers_pay_return_due(Request $request)
     {
        $user_auth = auth()->user();
		if ($user_auth->can('pay_purchase_return_due')){
