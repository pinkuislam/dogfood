...
class ProductsController extends Controller
{
     ...
     public function get_product_datatable(Request $request)
     {
        ...
        if (!$user_auth->can('products_view')){
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('sku'), function ($query) use ($request) {
                    return $query->where('products.sku', 'LIKE', "%{$request->sku}%")
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search.value'), function ($query) use ($request) {
                    return $query->where('products.name', 'LIKE', "%{$request->input('search.value')}%")
                        ->orWhere('products.sku', 'LIKE', "%{$request->input('search.value')}%")
            ...
            foreach ($products as $product) {
                $item['id'] = $product->id;                   
                $item['category'] = $product['category'] ? $product['category']->name : '-';
                $item['brand'] = $product['brand'] ? $product['brand']->name : '-';

                if($product->type == 'is_single' || $product->type == 'is_raw'){

                    $item['type']  = $product->type == 'is_raw' ? 'Raw Material' : 'Standard Product';
                    ...
                    $ProductWarehouse_total_qty = ProductWarehouse::where('product_id', $product->id)
                    ...
                    $item['quantity'] = $ProductWarehouse_total_qty .' '.$product['unit']->ShortName;

                }
                else if($product->type == 'is_variant' || $product->type == 'is_subscription' || $product->type == 'is_finish'){

                    $item['type'] = $product->type == 'is_subscription' ? 'Subscription' : 'Variable';
                    
                    if($product->type == 'is_finish'){
                        $item['type'] = 'Finish Product';
                    }
                ...
                else if($product->type == 'is_service'){
                    $item['type'] = 'Service';
                    $item['name'] = $product->name;
                    $item['sku'] = $product->sku;
                    $item['cost'] = '----';
                ...
                $item['image'] = '<div class="avatar mr-2 avatar-md"><img class="img-thumbnail" src="'.$url.'" alt=""></div>';

                $item['action'] = '<button type="button" class="btn bg-transparent _r_btn border-0" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
                ...
                if ($user_auth->can('products_edit')){
                    $item['action'] .= '<a class="dropdown-item" href="'.url('products/products/' .$product->id. '/edit').'" id="' .$product->id. '"><i class="nav-icon far fa-edit text-success font-weight-bold m3-2"></i> ' .trans('translate.edit_product').'</a>';
                }   
                if ($user_auth->can('products_delete')){
                    $item['action'] .= '  <a class="delete dropdown-item cursor-pointer" id="' .$product->id. '"><i class="nav-icon far fa-times-circle text-danger font-weight-bold mr-3"></i> ' .trans('translate.delete_product').'</a>';
                }
                $item['action'] .= '</div>';
    ...
    public function create()
    {
		...
		if ($user_auth->can('products_add')){
            ...
            $raw_materials = Product::with('unit')->where('deleted_at', null)->where('type', 'is_raw')->orderBy('name', 'ASC')->get()->toArray();
            //$subscription_plans = [['value'=>10,'name'=>'10x Days Pack', 'value'=>28,'name'=>'28x Days Pack']];
            //echo '<pre>';print_r($raw_materials);exit;
            $currency = $this->currency;
            $symbol_placement = $this->symbol_placement;

            return view('products.create_product', compact('categories','brands','units','raw_materials', 'currency', 'symbol_placement'));
    ...
    public function store(Request $request)
    {
		...
		if ($user_auth->can('products_add')){
            ...
            if ($request->type == 'is_variant' || $request->type == 'is_subscription' || $request->type == 'is_finish') {
                $productRules['variants'] = [
                    'required',
                    function ($attribute, $value, $fail) use ($request) {
                        // check if array is not empty
                        if (empty($value)) {
                            $fail('The variants option is required.');
                            return;
                        }

                        // check for duplicate skus in variants array
                        ...
                        if($variants){
                            foreach ($variants as $variant) {
                                if (!array_key_exists('text', $variant) || empty($variant['text'])) {
                                    ...
                                    return;
                                }else if(!array_key_exists('sku', $variant) || empty($variant['sku'])) {
                                    $fail('Variant sku cannot be empty.');
                                    return;
                                }else if(!array_key_exists('size', $variant) || empty($variant['size'])) {
                                    $fail('Variant size cannot be empty.');
                                    return;
                        ...
                        //check if sku Duplicate
                        if (count(array_unique($skus)) !== count($skus)) {
                            $fail('Duplicate sku found in variants option.');
                            return;
                        }

                        // check for duplicate skus in product_variants table
                        ...
                        if (!empty($duplicateSkus)) {
                            $fail('This sku: '.implode(', ', $duplicateSkus).' already used');
                        }

                        // check for duplicate skus in products table
                        $duplicateSkus_products = DB::table('products')
                            ->whereIn('sku', $skus)
                            ->whereNull('deleted_at')
                            ->pluck('sku')
                            ->toArray();
                        if (!empty($duplicateSkus_products)) {
                            $fail('This sku : '.implode(', ', $duplicateSkus_products).' already used');
                        }
                    },
                ];
            }
            ...
            $validatedData = $request->validate($productRules, [
                'sku.unique'   => 'Product sku already used.',
            ...
            \DB::transaction(function () use ($request) {
                ...
                //-- check if type is_single or is_raw
                if($request['type'] == 'is_single' || $request['type'] == 'is_raw'){
                    $Product->price = (float)$request['price'];
                    $Product->cost  = (float)$request['cost'];

                    $Product->unit_id = $request['unit_id'];
                    $Product->unit_sale_id = $request['unit_id'];
                    $Product->unit_purchase_id = $request['unit_id'];
                ...
                else if($request['type'] == 'is_variant' || $request['type'] == 'is_subscription' || $request['type'] == 'is_finish'){
                    ...
                    $manage_stock = $request['type'] == 'is_finish' ? 0 : 1;
                ...
                // Store Finish Product
                $Product_materials_data = array();
                if ($request['type'] == 'is_finish') {
                    ...
                    ProductMaterial::insert($Product_materials_data);
                }
    ...
    public function show_product_data($id , $variant_id)
    {
        ...
        //product single
        if($Product_data['type'] == 'is_single' || $Product_data['type'] == 'is_raw'){
        ...
        //product is_variant
        }elseif($Product_data['type'] == 'is_variant' || $Product_data['type'] == 'is_finish'){
            ...
            $item['sku'] = $product_variant_data['sku'];
    ...
    public function edit($id)
    {
		...
		if ($user_auth->can('products_edit')){
            ...
            if ($Product->type == 'is_variant' || $Product->type == 'is_subscription' || $Product->type == 'is_finish') {
                if($Product->type == 'is_finish'){
                    $productsMaterials = ProductMaterial::where('product_id', $id)
                    ->where('deleted_at', null)
                    ...
                    foreach ($productsMaterials as $material) {
                        $material_item = array();
                        $material_item['id'] = $material['material_id'];
                        $material_item['product_id'] = $material['product_id'];
                        $material_item['name'] = $material['product']['name'];
                        $material_item['unit'] = $material['product']['unit']['name'];
                        $material_item['cost'] = $material['product']['cost'];
                        $material_item['price'] = $material['product']['price'];
                        $item['ProductMaterial'][] = $material_item;
                    }
                }
                ...
                foreach ($productsVariants as $variant) {
                    ...
                    $variant_item['size'] = $variant->size;
		...
		if ($user_auth->can('products_edit')){
            ...
            try {
                ...
                if ($request->type == 'is_variant') {
                    $productRules['variants'] = [
                        ...
                        function ($attribute, $value, $fail) use ($request, $id) {
                            ...
                            //check if sku Duplicate
                            if (count(array_unique($skus)) !== count($skus)) {
                                $fail('Duplicate skus found in variants array.');
                                return;
                            }
                ...
                \DB::transaction(function () use ($request, $id) {
                    ...
                    elseif($request['type'] == 'is_variant' || $request['type'] == 'is_finish'){
                    ...
                    if ($request['type'] == 'is_finish') {
                        if ($oldMaterials->isNotEmpty()) {
                            ...
                            foreach ($request['materials'] as $new_id) {
                                if (array_key_exists($var, $new_id)) {
                                    $new_materials_id[] = $new_id['id'];
                                } else {
                                    $new_materials_id[] = 0;
                                }
                            }
    
                            foreach ($oldMaterials as $key => $value) {
                                $old_materials_id[] = $value->material_id;
    
                                // Delete Old
                                if (!in_array($value->material_id, $new_materials_id)) {
                                    $ProductMaterial = ProductMaterial::findOrFail($value->id);
                                    $ProductMaterial->deleted_at = Carbon::now();
                                    $ProductMaterial->save();
                                }
                            }
    
                            foreach ($request['materials'] as $key => $material) {
                                if (array_key_exists($var, $material)) {
    
                                    $ProductMaterialDT = new ProductMaterial;
                                    //-- Field Required
                                    $ProductMaterialDT->product_id = $id;
                                    $ProductMaterialDT->material_id = $material['id'];
                                } else {
                                }
    
                                if (!in_array($new_materials_id[$key], $old_materials_id)) {
                                    $ProductMaterialDT->save();
                                } else {
                                }
                            }
                        }
                        ...
                        if ($oldVariants->isNotEmpty()) {
                            ...
                            foreach ($request['variants'] as $variant) {
                                ...
                                $ProductVarDT->sku = $variant['sku'];
                                $ProductVarDT->name = $variant['text'];
                                $ProductVarDT->size = $variant['size'];
                                ...
                                if ($warehouses) {
                                    ...
                                    ProductWarehouse::insert($ProductWarehouse_DT);
                                }
                            }
                        ...
                        if ($oldMaterials->isNotEmpty()) {
                            foreach ($oldMaterials as $old_var) {
                                $ProductMaterial = ProductMaterial::where('id', $old_var['id'])
                                    ->update([
                                        'deleted_at' => Carbon::now(),
                                    ]);
                            }
    ...
    {
        $data = [];
        $ProductWarehouse_data = ProductWarehouse::with('warehouse', 'product', 'productVariant')
        ...
        foreach ($ProductWarehouse_data as $ProductWarehouse) {

            if ($ProductWarehouse->product_variant_id) {
                
                $item['product_variant_id'] = $ProductWarehouse->product_variant_id;
                $item['sku'] = $ProductWarehouse['productVariant']->sku;
                $item['Variant'] = '['.$ProductWarehouse['productVariant']->name . ']' . $ProductWarehouse['product']->name;
                $item['name'] = '['.$ProductWarehouse['productVariant']->name . ']' . $ProductWarehouse['product']->name;
                $item['barcode'] = $ProductWarehouse['productVariant']->sku;


                $product_price = $ProductWarehouse['productVariant']->price;
                
            } else {
                $item['product_variant_id'] = null;
                $item['Variant'] = null;
                $item['sku'] = $ProductWarehouse['product']->sku;
                $item['name'] = $ProductWarehouse['product']->name;
                $item['barcode'] = $ProductWarehouse['product']->sku;

                $product_price =  $ProductWarehouse['product']->price;
            }

            $item['id'] = $ProductWarehouse->product_id;
            $item['product_type'] = $ProductWarehouse['product']->type;
            $item['qty_min'] = $ProductWarehouse['product']->qty_min;
            $item['Type_barcode'] = $ProductWarehouse['product']->Type_barcode;
            $item['image'] = $ProductWarehouse['product']->image;

            if($ProductWarehouse['product']['unitSale']){

                if($ProductWarehouse['product']['unitSale']->operator == '/') {
                    $item['qty_sale'] = $ProductWarehouse->qty * $ProductWarehouse['product']['unitSale']->operator_value;
                    $price = $product_price / $ProductWarehouse['product']['unitSale']->operator_value;
                ...
                $item['qty_sale'] = $ProductWarehouse->qty;
                $price = $product_price;
            }

            if($ProductWarehouse['product']['unitPurchase']) {

                if($ProductWarehouse['product']['unitPurchase']->operator == '/') {
                    $item['qty_purchase'] = round($ProductWarehouse->qty * $ProductWarehouse['product']['unitPurchase']->operator_value, 5);
                ...
                $item['qty_purchase'] = $ProductWarehouse->qty;
            }

            $item['manage_stock'] = $ProductWarehouse->manage_stock;
            $item['qty'] = $ProductWarehouse->qty;
            $item['unitSale'] = $ProductWarehouse['product']['unitSale']?$ProductWarehouse['product']['unitSale']->ShortName:'';
            $item['unitPurchase'] = $ProductWarehouse['product']['unitPurchase']?$ProductWarehouse['product']['unitPurchase']->ShortName:'';
      ...
      public function Products_Alert(request $request)
      {
          ...
          $ProductWarehouse_data = ProductWarehouse::with('warehouse', 'product', 'productVariant')
              ->join('products', 'ProductWarehouse.product_id', '=', 'products.id')
              ...
              })->where('ProductWarehouse.deleted_at', null)->get();
  
          $data = [];
  
          if ($ProductWarehouse_data->isNotEmpty()) {
  
              foreach ($ProductWarehouse_data as $ProductWarehouse) {
                  if ($ProductWarehouse->qty <= $ProductWarehouse['product']->stock_alert) {
                      if ($ProductWarehouse->product_variant_id) {
                          $item['sku'] = $ProductWarehouse['productVariant']->name . '-' . $ProductWarehouse['product']->sku;
                      } else {
                          $item['sku'] = $ProductWarehouse['product']->sku;
                      }
                      $item['quantity'] = $ProductWarehouse->qty;
                      $item['name'] = $ProductWarehouse['product']->name;
                      $item['warehouse'] = $ProductWarehouse['warehouse']->name;
                      $item['stock_alert'] = $ProductWarehouse['product']->stock_alert;
                      $data[] = $item;
                  }
              }
          }
		...
		if ($user_auth->can('products_add')){
            ...
            foreach ($product_array[0] as $key => $value) {
                ...
                if($value['name'] != ''){
                    ...
                    return back()->with('error','Product name does not exist!');
                }

                 //--Product sku
                 if($value['sku'] != ''){
                    if (Product::where('sku', $value['sku'])->where('deleted_at', '=', null)->exists()) {
                        return back()->with('error','Product'.' "'.$value['name'].'" '.'duplicate!');
                    }else{
                        $row['sku'] = $value['sku'];
                    }
                }else{
                    return back()->with('error','Product'.' "'.$value['name'].'" '.'n\'does not exist!');
                }
            ...
            if(count($product_array[0]) != count(array_unique($sku_array))){
                $duplicate = true;
                return back()->with('error','Duplicate SKU!');
            }
    ...
    public function generate_random_code()
    {
       ...
       if (Product::where('sku', $gen_code)->exists()) {
