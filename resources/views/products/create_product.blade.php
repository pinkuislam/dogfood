                ...
                <div class="card-body">
                    <div class="row">
                        <div class="form-group col-md-4 mb-3">
                            {{--,{label: 'Variable Product', value: 'is_variant'},{label: 'Variable Subscription', value: 'is_subscription'}--}}
                            <label>{{ __('translate.Product_Type') }} <span class="field_required">*</span></label>
                            <v-select placeholder="Product Type" v-model="product.type" :reduce="(option) => option.value" :options="[{label: 'Raw Material', value: 'is_raw'},{label: 'Finish Product', value: 'is_finish'},{label: 'Simple Product', value: 'is_single'}]">
                            </v-select>
                        ...
                        <div class="form-group col-md-4" v-if="product.type == 'is_single' || product.type == 'is_raw'">
                            <label for="cost">{{ __('translate.Product_Cost') }} <span class="field_required">*</span></label>
                            <input type="text" class="form-control" id="cost" placeholder="{{ __('translate.Enter_Product_Cost') }}" v-model="product.cost">
                        ...
                        <div class="form-group col-md-4" v-if="product.type == 'is_single' || product.type == 'is_raw'">
                            ...
                            <input type="text" class="form-control" id="price" placeholder="{{ __('translate.Enter_Product_Price') }}" v-model="product.price">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <v-select @input="Selected_Unit" placeholder="{{ __('translate.Choose_Unit_Product') }}" v-model="product.unit_id" :reduce="label => label.value" :options="units.map(units => ({label: units.name, value: units.id}))">
                            </v-select>
...
<script>
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            // Create new Product
            Create_Product() {
                if ((this.product.type == 'is_variant' || this.product.type == 'is_subscription') && this.variants.length <= 0) {
                    toastr.error('The variants option is required.');
                }
                else{
                    NProgress.start();
                    NProgress.set(0.1);
                    ...
                    if ((self.product.type == 'is_variant' || self.product.type == 'is_subscription' || self.product.type == 'is_finish') && self.variants.length > 0) {
                    ...
                    if (self.materials.length) {
                        self.data.append("materials", JSON.stringify(self.materials));
                    }
                    ...
                    axios
                    .post("{{ url('products/products') }}", self.data)
                    .then(response => {
