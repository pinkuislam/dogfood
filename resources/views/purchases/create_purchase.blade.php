  ...
  <div class="col-lg-12 mb-3">
    <validation-observer ref="create_purchase">
      <form @submit.prevent="Submit_Purchase">
            ...
            <div class="row">
             ...
             <div class="col-md-12 mb-5 mt-3">
              <div id="autocomplete" class="autocomplete">
                  <input placeholder="{{ __('translate.Scan_Search_Product_by_Code_Name') }}" @input='e => search_input = e.target.value' @keyup="search(search_input)" @focus="handleFocus" @blur="handleBlur" ref="product_autocomplete" class="autocomplete-input" />
                  <ul class="autocomplete-result-list" v-show="focused">
                      <li class="autocomplete-result" v-for="product_fill in product_filter" @mousedown="SearchProduct(product_fill)">@{{getResultValue(product_fill)}}</li>
                  </ul>
              </div>
            </div>
                  ...
                  <table class="table table-hover table-md">
                    ...
                    <tbody>
                      <tr v-if="details.length <=0">
                        <td colspan="8" class="text-center">{{ __('translate.No_data_Available') }}</td>
                      </tr>
                      <tr v-for="detail in details">
                        ...
                        <td>
                          ...
                          <br>
                          <span class="badge badge-gray-300">@{{detail.sku}}</span>
                        </td>
                        <td>{{$currency}} @{{formatNumber(detail.Net_cost, 2)}}</td>
                        <td>
                          <span class="badge badge-warning">@{{detail.stock}} @{{detail.unitPurchase}}</span>
                        </td>
                        <td>
                          <div class="d-flex align-items-center">
                            <span class="increment-decrement btn btn-light rounded-circle" @click="decrement(detail ,detail.detail_id)">-</span>
                            <input class="fw-semibold cart-qty m-0 px-2" @keyup="Verified_Qty(detail,detail.detail_id)" :min="0.00" v-model.number="detail.quantity" >
                        ...
                        <td>
                          <a @click="Modal_Update_Detail(detail)" class="cursor-pointer ul-link-action text-success" title="Edit"><i class="far fa-edit"></i></a>
                          <a @click="delete_Product_Detail(detail.detail_id)" class="cursor-pointer ul-link-action text-danger" title="Delete"><i class="far fa-times-circle"></i></a>
                        </td>
                      </tr>
                    </tbody>
                  </table>
        ...
        <div class="card mt-5">
          <div class="card-body">
            <div class="row">
              ...
              <div class="form-group col-md-4">
                  <label for="Discount">{{ __('translate.Discount_Type') }} </label>
                  <select class="form-select" id="inputGroupSelect02" @change="Calculate_Total()" v-model="purchase.discount_type">
        ...
        <div class="row mt-3">
          <div class="col-lg-6">
            <button @click="Submit_Purchase" class="btn btn-primary" :disabled="SubmitProcessing" type="button">
              <span v-if="SubmitProcessing" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> <i class="far fa-check-circle me-2 font-weight-bold"></i> {{ __('translate.Submit') }}
            </button>
          </div>
        </div>
      </form>
            ...
            <form @submit.prevent="submit_Update_Detail">
              <div class="row">
                ...
                <div class="form-group col-md-6">
                  <validation-provider name="Product Cost " :rules="{ required: true , regex: /^\d*\.?\d*$/}" v-slot="validationContext">
                    <label for="Unit_cost">{{ __('translate.Product_Cost') }}
                      <span class="field_required">*</span></label>
                    <input :state="getValidationState(validationContext)" aria-describedby="Unit_cost-feedback" v-model.number="detail.Unit_cost" type="text" class="form-control">
                    <span class="error">@{{ validationContext.errors[0] }}</span>
                  </validation-provider>
                </div>
                ...
                <div class="form-group col-md-6">
                  <validation-provider name="Discount_Method" rules="required" v-slot="{ valid, errors }">
                    ...
                    <v-select placeholder="{{ __('translate.Choose_Method') }}" v-model="detail.discount_Method" :reduce="(option) => option.value" :options="[{label: 'Percent %', value: '1'},{label: 'Fixed', value: '2'}]">
                    </v-select>
                    <span class="error">@{{ errors[0] }}</span>
                  </validation-provider>
                </div>
                ...
                <div class="form-group col-md-6">
                  <validation-provider name="Discount" :rules="{ required: true , regex: /^\d*\.?\d*$/}"
                    ...
                    <input :state="getValidationState(validationContext)" aria-describedby="Discount-feedback" v-model="detail.discount" type="text" class="form-control">
                    <span class="error">@{{ validationContext.errors[0] }}</span>
                  </validation-provider>
                </div>
                ...
                <div class="form-group col-md-6">
                  <validation-provider name="UnitPurchase" rules="required" v-slot="{ valid, errors }">
                    ...
                    <v-select v-model="detail.purchase_unit_id" :reduce="label => label.value" placeholder="{{ __('translate.Choose_Unit_Purchase') }}" :options="units.map(units => ({label: units.name, value: units.id}))"></v-select>
                    <span class="error">@{{ errors[0] }}</span>
                  </validation-provider>
                </div>
                ...
                <div class="col-lg-12">
                  <button type="submit" :disabled="Submit_Processing_detail" class="btn btn-primary">
                    <span v-if="Submit_Processing_detail" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> {{ __('translate.Submit') }}
                  </button>
                </div>
              </div>
            </form>
...
<script type="text/javascript">
    $(function () {
        ...
        $(document).ready(function () {
            ...
            setTimeout(function(){
                app.purchase.warehouse_id = 1;
                app.Selected_Warehouse(1);
            },1000);
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            Get_Purchases_units(value) {
                axios
                .get("{{ url('products/get_sales_units?id=') }}" + value)
            ...
            Update_Detail() {
                ...
                for (var i = 0; i < this.details.length; i++) {
                    if (this.details[i].detail_id === this.detail.detail_id) {
                        ...
                        this.detail.Net_cost = this.detail.Unit_cost = Number((this.detail.Unit_cost).toFixed(2));
                        this.details[i].Unit_cost = this.detail.Unit_cost;
                        this.details[i].Net_cost = this.detail.Net_cost;
            ...
            SearchProduct(result) {
                this.product = {};
                if (this.details.length > 0 && this.details.some(detail => detail.sku === result.sku)) {
                    toastr.error('{{ __('translate.Product_Already_added') }}');
                } 
                else {
                    this.product.sku = result.sku;
                    this.product.quantity = 1;
                    this.product.stock = result.qty_purchase;
            ...
            Get_Products_By_Warehouse(id) {
                ...
                NProgress.set(0.1);
                axios
                .get("{{ url('products/products_by_warehouse') }}/" + id + "?stock=" + 0)
                .then(response => {
