        ...
        <div class="card">
            <div class="card-body">
                <div class="text-end mb-3">
                    @can('suppliers_add')
                         <a class="btn btn-outline-primary btn-md m-1 new_Supplier"><i class="fas fa-plus-circle me-2 font-weight-bold"></i>
                            ...
                            <div class="col-md-6">
                                <button type="submit" class="btn btn-primary" :disabled="SubmitProcessing">
                                    <span v-if="SubmitProcessing" class="spinner-border spinner-border-sm" role="status"
                                        aria-hidden="true"></span> <i class="far fa-check-circle me-2 font-weight-bold"></i>
                                    {{ __('translate.Submit') }}
                                </button>
                                ...
                                <div class="col-lg-6">
                                    <button type="submit" class="btn btn-primary" :disabled="paymentProcessing">
                                        <span v-if="paymentProcessing" class="spinner-border spinner-border-sm"
                                            role="status" aria-hidden="true"></span> <i class="far fa-check-circle me-2 font-weight-bold"></i>
                                        {{ __('translate.Submit') }}
                                    </button>
       ...
       function suppliers_datatable(){
        ...
        $(document).bind('event_supplier', function (e) {
         ...
         $(document).on('click', '.payment_purchase', function () {
            ...
            app.get_supplier_debt_total(id);
            setTimeout(() => {
                NProgress.done();
                if(app.total_debt > 0){
        ...
        $(document).bind('event_create_payment_purchase', function (e) {
         ...
         $(document).on('click', '.payment_return', function () {
            ...
            app.get_supplier_debt_return_total(id);
            setTimeout(() => {
                NProgress.done();
                if(app.return_due > 0){
...
<script>
    ...
    var app = new Vue({
          ...
          methods: {
            ...
            get_supplier_debt_return_total(id) {
                axios
                    .get("{{ url('get_supplier_debt_return_total') }}/" + id)
                    .then(response => {
                    this.return_due = parseFloat(response.data.return_due);
                    this.payment_return.supplier_id = id;
            ...
            Submit_Pay_return_due() {
                ...
                axios
                    .post("{{ url('suppliers_pay_return_due') }}", {
                        date: this.payment_return.date,
                        supplier_id: this.payment_return.supplier_id,
            ...
            get_supplier_debt_total(id) {
                axios
                    .get("{{ url('get_supplier_debt_total') }}/" + id)
                    .then(response => {
                    this.total_debt = parseFloat(response.data.total_debt);
                    this.payment.supplier_id = id;
            ...
            Get_Data_Edit(id) {
                axios
                .get("{{ url('products/suppliers') }}/"+id+"/edit")
                .then(response => {
              ...
              Create_Supplier() {
                  ...
                  axios.post("{{ url('products/suppliers') }}", {
             ...
             //----------------------- Update supplier ---------------------------\\
              Update_Supplier() {
                  var self = this;
                  self.SubmitProcessing = true;
                  axios.put("{{ url('products/suppliers') }}/" + self.supplier.id, {
                      name: self.supplier.name,
                      email: self.supplier.email,
                      country: self.supplier.country,
                      city: self.supplier.city,
            ...
            Remove_Supplier(id) {
                swal({
                    ...
                    axios
                        .delete("{{ url('products/suppliers') }}/" + id)
                        .then(() => {
